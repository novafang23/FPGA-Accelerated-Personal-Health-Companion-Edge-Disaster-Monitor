...
for name, sig_type, val in signals:
    ...
    if sig_type == "digital":
        ...
        for s_start, s_end, s_label, s_color in states:
            rect = patches.Rectangle((s_start, base_y), s_end - s_start, 0.8, facecolor=s_color, edgecolor='#ffffff', alpha=0.85)
            ax.add_patch(rect)
            ax.text((s_start + s_end)/2, base_y + 0.35, s_label, color='white', fontsize=7.5, weight='bold', ha='center', va='center')
    elif sig_type == "bus":
        # Bus representation
...
# Annotations
ax.annotate('Decoupled AXI Write Handshake (AW & W in separate cycles)', xy=(20, 9.5), xytext=(35, 11.5),
            arrowprops=dict(facecolor='#38bdf8', shrink=0.05, width=1.5, headwidth=6),
            color='#38bdf8', fontsize=8.5, weight='bold', bbox=dict(boxstyle="round,pad=0.3", fc="#1e293b", ec="#38bdf8"))

ax.annotate('Systolic Peak Detected → irq_beat 1-cycle pulse', xy=(70, 2.5), xytext=(55, 4.5),
            arrowprops=dict(facecolor='#22c55e', shrink=0.05, width=1.5, headwidth=6),
            color='#22c55e', fontsize=8.5, weight='bold', bbox=dict(boxstyle="round,pad=0.3", fc="#1e293b", ec="#22c55e"))

ax.set_xlim(-25, 105)
ax.set_ylim(-0.5, len(signals) * 1.3 + 0.5)
ax.set_title("GTKWave Timing Simulation: Decoupled AXI4-Lite Handshake & Systolic Peak Detection (50 MHz / 20ns)", color='#ffffff', fontsize=11, weight='bold', pad=15)
ax.axis('off')

plt.tight_layout()
plt.savefig(r"c:\Users\abhin\OneDrive\Desktop\verilog\SIH\waveform_snapshot.png", dpi=150, facecolor=fig.get_facecolor(), bbox_inches='tight')
plt.close()
