...
| Docs claim 443 LUT5s | **partly fixed** | The tracked evidence log was from the **superseded 4-bit parallel build** (port signature 8 in / 6 out, PLL 1/1), not the SPI design. The SPI fitter run measures **222/1120 (19.82%), 121 FFs, 40/140 CLBs, PLL 0/1** — not the 202/123/37 previously quoted here. Corrected in `README.md`, `docs/MIGRATION.md`, `hardware/shrikefi/README.md` and `synthesis_evidence/`. **Still quoting 443:** `docs/MASTER_PROJECT_GUIDE.md`, `docs/theory/THEORY_NOTES.md`, `docs/theory/theory_notes_print.html`, `scripts/generate_sih_presentation.py`, `scripts/generate_sih_official_presentation.py`, `hardware/shrikefi/presentation.gtkw`. |
...
| Bitstream no longer matches the RTL | **open** | The shared-source refactor and the power-on reset both change the netlist. `firmware/shrikefi/forgefpga_bitstream.h` still holds the pre-refactor build. Re-run the ForgeFPGA fitter, then `convert_bitstream.py`. Detector behaviour is unaffected (differential simulation: 30,000 samples, 0 mismatches); the reset distribution is the change. |
