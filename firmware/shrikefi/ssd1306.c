/*
 * ssd1306.c — SSD1306 OLED Display Driver Implementation (ESP-IDF)
...
int ssd1306_init(ssd1306_t *disp, esp32_i2c_handle_t *i2c, uint8_t addr) {
    ...
    /* Standard init sequence for SSD1306 */
    const uint8_t init_cmds[] = {
        ...
        SSD1306_CHARGE_PUMP_ENABLE,          /* Internal charge pump (0x14) */
    ...
    uint8_t cmd_buf[sizeof(init_cmds) + 1];
    cmd_buf[0] = 0x00; /* Stream command prefix */
    memcpy(&cmd_buf[1], init_cmds, sizeof(init_cmds));
    if (esp32_i2c_hal_write_raw(disp->addr, cmd_buf, sizeof(cmd_buf)) != I2C_HAL_SUCCESS) {
        return -1;
    }

    disp->initialized = 1;

    /* Clear framebuffer and update display */
    ssd1306_clear(disp);
    ssd1306_update(disp);
...
void ssd1306_clear(ssd1306_t *disp) {
    if (!disp) return;
    memset(disp->framebuf, 0x00, sizeof(disp->framebuf));
}

int ssd1306_update(ssd1306_t *disp) {
    if (!disp || !disp->initialized) return -1;

    /* Set column and page address ranges via command stream */
    uint8_t setup_cmds[] = {
        0x00,
        SSD1306_SET_COL_ADDR, 0x00, 0x7F,
        SSD1306_SET_PAGE_ADDR, 0x00, 0x07
    };
    if (esp32_i2c_hal_write_raw(disp->addr, setup_cmds, sizeof(setup_cmds)) != I2C_HAL_SUCCESS) {
        return -1;
    }
    ...
    for (int offset = 0; offset < total; offset += chunk_size) {
        uint8_t buf[33];  /* 1 control byte + 32 data bytes */
        buf[0] = SSD1306_DATA_STREAM;
        int remaining = total - offset;
        int len = (remaining < chunk_size) ? remaining : chunk_size;
        memcpy(&buf[1], &disp->framebuf[offset], len);
        if (esp32_i2c_hal_write_raw(disp->addr, buf, (uint16_t)(len + 1)) != I2C_HAL_SUCCESS) {
            return -1;
        }
    }
    return 0;
...
void ssd1306_draw_string(ssd1306_t *disp, int x, int y, const char *str) {
    ...
    int cursor_x = x;
    while (*str) {
        if (cursor_x + 6 > SSD1306_WIDTH) break;
